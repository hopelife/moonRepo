...
### 아래 첨자	y_m | ``아래 첨자	y_m``
### x_s-x_D | ``x_s-x_D``
### 다문자 첨자	a^{2+2} | ``다문자 첨자	a^{2+2}``
### a_{i,j} | ``a_{i,j}``
### 위 아래 첨자 동시에	x_2^3 | ``위 아래 첨자 동시에	x_2^3``
### 위 가운데 첨자	\overset{x}{P} | ``위 가운데 첨자	\overset{x}{P}``
### 전치 위,아래 첨자 동시에	{}_{b}^{a}X | ``전치 위,아래 첨자 동시에	{}_{b}^{a}X``
### 전치,후치,위,아래 첨자 동시에	_{c}^{a}C_{d}^{b} | ``전치,후치,위,아래 첨자 동시에	_{c}^{a}C_{d}^{b}``
### 전,후치,가운데,위,아래 첨자 동시에	\underset{y}{\overset{x}{_{c}^{a}C_{d}^{b}}} | ``전,후치,가운데,위,아래 첨자 동시에	\underset{y}{\overset{x}{_{c}^{a}C_{d}^{b}}}``
### 미분 (옳음)	x' | ``미분 (옳음)	x'``
### 미분 (HTML의 경우 틀림)	x^\prime | ``미분 (HTML의 경우 틀림)	x^\prime``
### 미분 (PNG의 경우 틀림)	x\prime | ``미분 (PNG의 경우 틀림)	x\prime``
### 시그마	\sum_{k=1}^N k^2 | ``시그마	\sum_{k=1}^N k^2``
### 곱기호	\prod_{i=1}^N x_i | ``곱기호	\prod_{i=1}^N x_i``
### 극한	\lim_{n \to \infty}x_n | ``극한	\lim_{n \to \infty}x_n``
### 적분	\int_{-N}^{N} e^x\, dx | ``적분	\int_{-N}^{N} e^x\, dx``
### 선적분	\oint_{C} x^3\, dx + 4y^2\, dy | ``선적분	\oint_{C} x^3\, dx + 4y^2\, dy``
### 분수, 행렬, 여러행
### 기능	문법 | ``기능	문법``
### 분수	\frac{2}{4} or {2 \over 4} | ``분수	\frac{2}{4} or {2 \over 4}``
### 이항
### 계수	{n \choose k} | ``계수	{n \choose k}``
### 행렬	\begin{pmatrix} x & y \\ z & v \end{pmatrix} | ``행렬	\begin{pmatrix} x & y \\ z & v \end{pmatrix}``
### \begin{bmatrix}
### 0 & \cdots & 0 \\
### \vdots & \ddots & \vdots \\
### 0 & \cdots & 0
### \end{bmatrix}
### {\displaystyle {\begin{bmatrix}0&\cdots &0\\\vdots &\ddots &\vdots \\0&\cdots &0\end{bmatrix}}} {\begin{bmatrix}0&\cdots &0\\\vdots &\ddots &\vdots \\0&\cdots &0\end{bmatrix}}
...
### \end{cases}
### {\displaystyle f(n)={\begin{cases}n/2,&{\mbox{if }}n{\mbox{ is even}}\\3n+1,&{\mbox{if }}n{\mbox{ is odd}}\end{cases}}} f(n)={\begin{cases}n/2,&{\mbox{if }}n{\mbox{ is even}}\\3n+1,&{\mbox{if }}n{\mbox{ is odd}}\end{cases}}
### 두줄
### 이상의
### 방정식	
### \begin{matrix}
...
###        &=& n^2 + 2n + 1
### \end{matrix}
### {\displaystyle {\begin{matrix}f(n+1)&=&(n+1)^{2}\\\ &=&n^{2}+2n+1\end{matrix}}} {\begin{matrix}f(n+1)&=&(n+1)^{2}\\\ &=&n^{2}+2n+1\end{matrix}}
### 글꼴
### 그리스어
...
### 괄호 쓰기
### 기능	문법 | ``기능	문법``
### 좋지 않음.	( \frac{1}{2} ) | ``좋지 않음.	( \frac{1}{2} )``
### 좋음.	\left( \frac{1}{2} \right) | ``좋음.	\left( \frac{1}{2} \right)``
### \left 와 \right 를 사용하여, 여러가지 괄호를 사용할 수 있습니다.
### 
### 기능	문법 | ``기능	문법``
### 괄호	\left( A \right) | ``괄호	\left( A \right)``
### 사각 괄호	\left[ A \right] | ``사각 괄호	\left[ A \right]``
### 집합괄호 | ``집합괄호``
### (주의: \ 필요) | ``(주의: \ 필요)``
### 부등호 괄호	\left\langle A \right\rangle | ``부등호 괄호	\left\langle A \right\rangle``
### 바	\left| A \right| | ``바	\left| A \right|``
| \left. 혹은 \right. 라고 쓰면, 그 쪽 괄호는 나타나지 않습니다.	\left. {A \over B} \right\} \to X | ``\left. 혹은 \right. 라고 쓰면, 그 쪽 괄호는 나타나지 않습니다.	\left. {A \over B} \right\} \to X `` |
### 내림수와 올림수 기호	\lfloor\sqrt{n}\rfloor \lceil\sqrt{n}\rceil | ``내림수와 올림수 기호	\lfloor\sqrt{n}\rfloor \lceil\sqrt{n}\rceil``
### 빈칸조정
### TeX은 빈칸의 크기를 자동으로 조정합니다. 특별히 조정이 필요한 경우는 다음을 사용하면 됩니다.
### 
### 기능	문법 | ``기능	문법``
### double quad space	a \qquad b | ``double quad space	a \qquad b``
### quad space	a \quad b | ``quad space	a \quad b``
### text space	a\ b | ``text space	a\ b``
### large space	a\;b | ``large space	a\;b``
### medium space	a\>b | ``medium space	a\>b``
### small space	a\,b | ``small space	a\,b``
### no space	ab | ``no space	ab``
### negative space	a\!b | ``negative space	a\!b``